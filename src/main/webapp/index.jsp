...
<ul id="top-navigation">
    <li><a class="active" href="<c:url value="index.do"/>">Benchmark</a></li>
    <li><a href="<c:url value="index.do?training"/>">Training</a></li>
</ul>
<div id="container">
...
<div id="control">
    ...
    <script type="text/javascript">
        ...
        $('#settingsForm').submit(function () {
            var values = {};
            var settingGenerated = false;
            $.each($('#settingsForm :input').serializeArray(), function (i, field) {
                if (!$.isNumeric(field.value)) {
                    if(values['preOpeningRunTime'] == undefined || values['tradingRunTime'] == undefined){
                        alert("invalid value: " + field.value + " for field: " + field.name);
                        return false;
                    }else{
                        if(field.name == 'preOpeningOrders'){
                            values[field.name] = 50;
                        }
                        else if(field.name == 'matchPercent'){
                            values[field.name] = 30;
                        }else if(field.name == 'totalBuyOrders' || field.name == 'totalSellOrders'){
                            values[field.name] = parseInt(((values['preOpeningRunTime'] + values['tradingRunTime']) * 60 * 100000)/2);
                        }
                        settingGenerated = true;
                    }
                }else{
            ...
            if (values['preOpeningRunTime'] + values['tradingRunTime'] > 60) {
                alert("Run time is greater than an hour!");
                return false;
            }
            else if (((values['totalBuyOrders']+values['totalSellOrders']) /
                    ((values['preOpeningRunTime'] * 60) + (values['tradingRunTime'] * 60))) > 100000) {
                alert("Throughput will be more than 100,000 messages/second!");
                return false;
            }
            else if (values['preOpeningOrders'] > 50) {
                alert("Pre-opening orders more than 50% of total!");
                return false;
            }
            else if (values['matchPercent'] < 30) {
                alert("Match percent smaller than 30%!");
                return false;
            }
            if(settingGenerated){
