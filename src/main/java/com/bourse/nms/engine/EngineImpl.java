...
/*
        try {
            dbInterface.insertSessionData(startTime.get(), System.currentTimeMillis(), settings.getPreOpeningTime(), settings.getTradingTime(),
                    settings.getBuyOrdersCount(), settings.getSellOrdersCount(), orderPutCounter.get(), settings.getPreOpeningOrdersPercent(),
                    settings.getMatchPercent(), meanPutOrderPerSeconds.get(), minimumPutOrderPerSeconds.get(), maximumPutOrderPerSeconds.get(),
                    meanTradePerSeconds.get(), minimumTradePerSeconds.get(), maximumTradePerSeconds.get(), totalTradesCost.get(),
                    settings.getPreOpeningLogPath(), settings.getTradingLogPath());
        } catch (SQLException e) {
            log.warn("exception while inserting log to db", e);
            throw new NMSException(NMSException.ErrorCode.INTERNAL_SERVER_ERROR, e.getMessage());
        }
*/
    }
    ...
    public void restart() {
        ...
        settings.setTradingLogPath(null);
        settings.setPreOpeningLogPath(null);
    ...
    @Override
    public TreeSet<Order> getQueueTop(OrderSide orderSide, int stockId, int limit) {
        final TreeSet<Order> response = new TreeSet<>();
        switch (orderSide){
            case BUY:
                if(buyQueues.containsKey(stockId)){
                    int i = 0;
                    final Iterator buyQueuesIter = buyQueues.get(stockId).iterator();
                    while (buyQueuesIter.hasNext() && i < limit){
                        response.add((Order) buyQueuesIter.next());
                        i++;
                    }
                }
                break;
            case SELL:
                if(sellQueues.containsKey(stockId)){
                    int i = 0;
                    final Iterator sellQueuesIter = sellQueues.get(stockId).iterator();
                    while (sellQueuesIter.hasNext() && i < limit){
                        response.add((Order) sellQueuesIter.next());
                        i++;
                    }
                }
                break;
            ...
            while (settings.getStatus().equals(Settings.EngineStatus.TRADING)
                ...
                if (buyOrder.getPrice() < sellOrder.getPrice()) {
                    ...
                    if (buyOrder.getTotalQuantity() > sellOrder.getTotalQuantity()) {
                        try {
                            putOrder(new Order((buyOrder.getTotalQuantity() - sellOrder.getTotalQuantity()),
                                    buyOrder.getSubscriberId(),
                                    buyOrder.getPrice(),
                                    buyOrder.getSubscriberPriority(), OrderSide.BUY), OrderSide.BUY, stockId, tradePrice);
                        } catch (NMSException e) {
                            log.warn("exception on putting remaining order", e);
                        }
                        //buyQueue.add();
                    } else if (buyOrder.getTotalQuantity() < sellOrder.getTotalQuantity()) {
                        try {
                            putOrder(new Order((sellOrder.getTotalQuantity() - buyOrder.getTotalQuantity()),
                                    sellOrder.getSubscriberId(),
                                    sellOrder.getPrice(),
                                    sellOrder.getSubscriberPriority(),OrderSide.SELL), OrderSide.SELL, stockId, tradePrice);
                        } catch (NMSException e) {
                            log.warn("exception on putting remaining order", e);
                        }
                        //sellQueue.add();
